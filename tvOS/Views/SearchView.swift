...
struct SearchView: View {
	...
	var body: some View {
		List(filteredMedias) { media in
			MediaItemView(media: media, playlistName: modelPlaylists.safelyAccessElement(at: selectedPlaylist)?.name ?? "Untitled")
		}
...
extension SearchView {
	private var filteredMedias: [Media] {
		...
		return allMedias.filter { $0.title.localizedCaseInsensitiveContains(searchQuery) }
	}
