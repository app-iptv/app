		...
		VStack {
			if filteredMovies?.isEmpty ?? true {
				ContentUnavailableView(
					"No Movies", systemImage: "film.stack",
					description: Text(
						"Either your playlist doesn't include movies, or you haven't yet added a playlist. You can do so in Settings."
					)
				)
				.padding()
			} else {
					...
					LazyVStack(alignment: .leading, spacing: 26) {
						ForEach(groups, id: \.self) { group in
							Section(LocalizedStringKey(group)) {
								MediaGroupView(
									medias: filteredMovies, group: group,
									isTV: false)
							}
						}
					}
...
extension MoviesAndTVShowsView {
	private var filteredMovies: [Media]? {
		modelPlaylists.safelyAccessElement(at: selectedPlaylist)?.medias.filter
		{ $0.attributes["media"] == "true" }
	}

	private var groups: [String] {
		var seen = Set<String>()
		let groups =
			filteredMovies?.compactMap {
