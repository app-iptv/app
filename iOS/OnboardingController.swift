...
struct OnboardingView: UIViewControllerRepresentable {
	...
	-> UIOnboardingViewController
	{
		...
		let onboardingVC = UIOnboardingViewController(
...
struct OnboardingController {
	...
	static func setUpFirstTitleLine() -> NSMutableAttributedString {
		.init(
			string: String(localized: "Welcome to"),
	...
	static func setUpSecondTitleLine() -> NSMutableAttributedString {
		.init(
			string: Bundle.main.displayName ?? "IPTV App",
			attributes: [
	...
	static func setUpFeatures() -> [UIOnboardingFeature] {
		return .init([
			.init(
				icon: .init(systemName: "film.stack")!,
				title: String(
					localized: "Watch your Movies or TV Channels"),
				description: String(
					localized:
						"Import your playlists and watch TV and movies wherever you are."
				)),
			.init(
				icon: .init(systemName: "icloud")!,
				title: String(localized: "iCloud Sync"),
				description: String(
					localized:
						"Your playlists securely sync across all of your devices using iCloud."
				)),
			.init(
				icon: .init(systemName: "list.bullet.below.rectangle")!,
				title: String(localized: "Electronic Program Guide"),
				description: String(
					localized:
						"Stay informed with EPG, a digital TV guide for all upcoming programs."
				)),
		])
	}
